...
class ZhongkuiHeadless:
    def __init__(self, model_path, mumu_manager_path, config_path, conf_thres=0.3):
        self.model = YOLO(model_path)
        self.capture = HeadlessCapture()
        self.mumu = Mumu(mumu_manager_path).select(1)
    ...
    def _move_towards(self, target_x, target_y, w, h):
        ...
        print(f"移动: ({start_x}, {start_y}) -> ({end_x}, {end_y})")
...
if __name__ == "__main__":
    ...
    agent = ZhongkuiHeadless(MODEL_PATH, MUMU_MANAGER_PATH, CONFIG_PATH)
    agent.run()
